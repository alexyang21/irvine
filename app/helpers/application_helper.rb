module ApplicationHelper
  ...
  def select_delivery_time
    ...
    times << Time.now
    times << Time.now.change({hour: 17})
    times << (((closing_time - opening_time)/1.minute)/30).round
    times << Time.at((current_time.to_f / 15.minutes).round * 15.minutes).in_time_zone("Pacific Time (US & Canada)")
    # current_time = Time.now
    # opening_time = Time.now.change({hour: 17})
    # closing_time = Time.now.change({hour: 21})
    ...
    #   puts "Current time rounded is #{current_time_rounded}"
    #   delivery_time = current_time_rounded + delivery_delay
    #   puts "Delivery time is #{delivery_time}"
